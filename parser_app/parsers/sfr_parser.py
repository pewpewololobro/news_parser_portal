...
class SfrParser(HTMLParser):
    ...
    def clean_text_from_date(self, text):
        """Очищает текст от даты в начале строки"""
        if not text:
            return text
        
        original_text = text
        
        # Паттерны дат, которые могут быть в начале текста
        date_patterns = [
            # 01.01.2024
            r'^\s*\d{2}\.\d{2}\.\d{4}\s*',
            r'^\s*\d{2}\.\d{2}\.\d{4}\s+',
            r'^\s*\d{2}\.\d{2}\.\d{4}год\s*',
            r'^\s*\d{2}\.\d{2}\.\d{4}г\.?\s*',
            r'^\s*\d{2}\.\d{2}\.\d{4}\s+г\.?\s*',
            r'^\s*\d{2}\.\d{2}\.\d{4}\s*[-–—]\s*',
            
            # 01.01.2024г без пробела (слитно)
            r'^\s*\d{2}\.\d{2}\.\d{4}г\.?(?=[А-Яа-я])',
            
            # 01 января 2024
            r'^\s*\d{1,2}\s+[а-я]+\s+\d{4}\s*',
            r'^\s*\d{1,2}\s+[а-я]+\s+\d{4}\s+года?\s*',
        ...
        # Удаляем даты с префиксами
        prefixes = ['Дата:', 'от:', 'опубликовано:', 'добавлено:', 'создано:']
        for prefix in prefixes:
            text = re.sub(rf'^\s*{prefix}\s*\d{{2}}\.\d{{2}}\.\d{{4}}\s*', '', text, flags=re.IGNORECASE)
        
        # Удаляем даты в формате "Сегодня, 10:00" или "Вчера, 15:30"
        text = re.sub(r'^\s*Сегодня,\s*\d{2}:\d{2}\s*', '', text)
        text = re.sub(r'^\s*Вчера,\s*\d{2}:\d{2}\s*', '', text)
        
        # Удаляем даты с днями недели
        weekdays = ['Понедельник', 'Вторник', 'Среда', 'Четверг', 'Пятница', 'Суббота', 'Воскресенье']
        for weekday in weekdays:
            text = re.sub(rf'^\s*{weekday},\s*\d{{2}}\.\d{{2}}\.\d{{4}}\s*', '', text)
            text = re.sub(rf'^\s*{weekday}\s+\d{{2}}\.\d{{2}}\.\d{{4}}\s*', '', text)
        
        # Удаляем временные метки в начале
        text = re.sub(r'^\s*\d{2}:\d{2}\s*', '', text)
        
        # Убираем лишние пробелы в начале
        text = text.lstrip()
        
        # Если первый символ - точка, запятая, тире и т.д., удаляем
        ...
        # Если после очистки текст стал пустым, возвращаем оригинал
        if not text or len(text.strip()) < 10:
            return original_text
        
        return text
    
    def extract_short_description(self, article):
        """Извлекает краткое описание новости из карточки"""
        desc_selectors = [
            '.news-desc',
            '.description',
            '.desc',
        ...
        for selector in desc_selectors:
            ...
            if desc_elem:
                ...
                if len(text) > 50:
                    return self.clean_text_from_date(text[:500])
    ...
    def extract_news_items(self, soup):
        ...
        for article in articles[:30]:
            ...
            description = self.extract_short_description(article)
    ...
    def extract_article_text(self, url, html=None):
        ...
        try:
            ...
            if not content:
                ...
                if paragraphs:
                    ...
                    if text_paragraphs:
                        print(f"      Найдено {len(text_paragraphs)} параграфов")
                        text = '\n\n'.join(text_paragraphs[:5])
                        text = self.clean_text_from_date(text)
                        return text[:2000]
            ...
            if content:
                ...
                lines = [line.strip() for line in text.splitlines() if line.strip() and len(line.strip()) > 30]
                result = '\n'.join(lines)[:2000]
                result = self.clean_text_from_date(result)
                return result
    ...
    def parse(self):
        try:
            ...
            for idx, item in enumerate(items[:30]):
                ...
                print(f"\n  [{idx+1}/{len(items)}] {title[14:60]}...")
                print(f"    Ссылка: {link}")
                
                # Получаем полный текст со страницы новости
                try:
                    full_text = self.extract_article_text(link)
                    # Очищаем текст от даты
                    full_text = self.clean_text_from_date(full_text)
                    description = full_text[:1000] if full_text else short_desc
                    if description:
                        print(f"    Текст получен: {len(description)} символов")
                    else:
                        print(f"    Текст не получен, используется краткое описание")
                except Exception as e:
                    print(f"    Ошибка получения текста: {e}")
