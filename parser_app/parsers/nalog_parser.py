...
class NalogParser(HTMLParser):
    """Парсер для сайта nalog.gov.ru/rn19/news/"""
    ...
    def extract_news_items(self, soup):
        ...
        # Ищем div с классом news-block news-block_grid js-grid
        news_block = soup.find('div', class_='news-block news-block_grid js-grid')
        
        if not news_block:
            # Пробуем другие варианты
            news_block = soup.select_one('.news-block.news-block_grid.js-grid, .news-block, .js-grid')
        
        if not news_block:
            print("  Не найден div.news-block.news-block_grid.js-grid")
            return []
        
        # Внутри ищем div с классом news-block__item js-grid-item
        news_items = news_block.find_all('div', class_='news-block__item js-grid-item')
        
        if not news_items:
            news_items = news_block.select('.news-block__item, .js-grid-item, .news-item')
        
        print(f"  Найдено news-block__item: {len(news_items)}")
        
        for item in news_items[:30]:
            # Ищем div с классом news-block__wrapper
            wrapper = item.find('div', class_='news-block__wrapper')
            
            if not wrapper:
                # Пробуем другие варианты
                wrapper = item.find('div', class_=re.compile(r'wrapper', re.I))
            
            if not wrapper:
                print("    Не найден news-block__wrapper, пропускаем")
                continue
            
            # Ищем div с классом news-block__text внутри wrapper
            text_div = wrapper.find('div', class_='news-block__text')
            
            if not text_div:
                text_div = wrapper.find('div', class_=re.compile(r'text', re.I))
            
            if not text_div:
                print("    Не найден news-block__text, пропускаем")
                continue
            
            # Извлекаем дату
            pub_date = self.extract_date(text_div)
            
            # Извлекаем заголовок (ссылка внутри text_div)
            title_elem = text_div.find('a', href=True)
            if not title_elem:
                title_elem = text_div.find(['h1', 'h2', 'h3', 'h4'])
            ...
            if not link:
                link_elem = text_div.find('a', href=True)
                if link_elem:
            ...
            if not link.startswith('http'):
                if link.startswith('/'):
                    link = 'https://www.nalog.gov.ru' + link
                else:
                    link = urljoin('https://www.nalog.gov.ru', link)
    ...
    def extract_date(self, text_div):
        """Извлекает дату из div с классом news-block__text"""
        # Ищем элементы с классами даты внутри text_div
        date_selectors = ['.date', '.time', '.news-date', '.post-date', '.published', '.data', '.news-block__date']
        for selector in date_selectors:
            date_elem = text_div.select_one(selector)
            if date_elem:
    ...
    def parse(self):
        try:
            ...
            with open('debug_nalog.html', 'w', encoding='utf-8') as f:
                f.write(html)
